...
| D41：更名 Ark 迁移（运营期） | 已完成 | 四阶段全收官：A/B/C 逐批对线推 main CI 绿；D 文档批加 **v1.0.0 已发版**（tag CI 绿、release 六资产双名同 digest、双 stable 段 200、部署位验收全绿含 replace_exe 首落修复 `ef44915`）；omc 三件批切回执已至（tools.ark 入册 seq 10、deploy 全切 ark、lan-linux 实证通）。过渡期运营收口（2026-09-14）：全舰队 ome 水位清零（omc 舰队对账装态调用态双零，残尾 lan-mac 与 lan-linux2 已升 ark 1.2.1 清 ome）；停 ome/ 面本仓三件落地（CI 撤 ome-* compat 双附、seed 撤 ome 段灌写、selfdeploy 停建 ome 别名并清理存量副本），catalog 删 tools.ome 节与 extract 切 ark-self 归 ohmycloud，桶内 ome/ 段对象删归 ohmycloud 回执；余 oma 回执待收 | 2026-09-14 |
