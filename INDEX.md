...
| S008 | `docs\research\S008-模型校验选型-serde反序列化加validator校验.md` | 模型校验主流栈事实面（serde 两步走、validator 0.21 实证、TOML 同型复用）与 ark 适配评估：当前不引入（HashMap 键为主力、两谓词单一权威），触发条件与迁入陷阱清单（D42 后续选型锚） |
...
| 编号 | 文件 | 覆盖主题 | 行级条目 |
| --- | --- | --- | --- |
| M101 | `docs\mistakes\M101-版本解析与下载-错误.md` | 版本解析与下载错误（REST、cdn、网络、哈希；M007 pin sha 不比 tag；M008 半截缓存复用；M013 滞后二进制复现已修行为；M019 云端 URL 拼成双 scheme 且被静默跳过掩盖；M026 回滚防护只挂显式路径漏默认自动路径；M028 沙盒 HOME 隔离切断 gh 文件凭据） | M007、M008、M013、M019、M026、M028 |
